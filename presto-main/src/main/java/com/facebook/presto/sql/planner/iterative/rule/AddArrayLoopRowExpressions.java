...
import static com.facebook.presto.spi.relation.SpecialFormExpression.Form.ARRAY_LOOP;
import static java.util.Objects.requireNonNull;

public class AddArrayLoopRowExpressions
        extends RowExpressionRewriteRuleSet
{
    public AddArrayLoopRowExpressions(Metadata metadata)
    {
            ...
            extends RowExpressionRewriter<Boolean>
    {
        ...
        {
            if (functionResolution.isArrayFunction(node.getFunctionHandle()) &&
                    node.getArguments().get(1) instanceof LambdaDefinitionExpression) {
                return new SpecialFormExpression(
                        ARRAY_LOOP,
                        node.getType(),
                        ImmutableList.of(node.getArguments().get(0), node.getArguments().get(1), node));
            }
            return node;
        }
