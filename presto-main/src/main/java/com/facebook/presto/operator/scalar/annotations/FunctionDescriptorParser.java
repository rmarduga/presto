...
public class FunctionDescriptorParser
{
    ...
    private static Function<Set<Subfield>, Optional<Set<Subfield>>> parseSubfieldTransformationFunction(StaticMethodPointer staticMethodPointer)
    {
        ...
        if (subfieldTransformationMethod.getReturnType() != Optional.class) {
            return null;
        }
        return (Set<Subfield> subfields) -> {
            try {
                return (Optional<Set<Subfield>>) subfieldTransformationMethod.invoke(null, subfields);
            }
    ...
    private static void checkSubfieldTransformFunctionTypeSignature(Method subfieldTransformationMethod)
    {
        ...
        {
            String errorMessage = "Subfield transformation function return type must be java.util.Optional<java.util.Set<com.facebook.presto.common.Subfield>>";
            Type type = subfieldTransformationMethod.getGenericReturnType();
            checkArgument(type instanceof ParameterizedType, errorMessage);
            final ParameterizedType optionalType = (ParameterizedType) type;
            checkArgument(optionalType.getRawType().equals(Optional.class), errorMessage);
            checkTypeIsSetOfSubfields(optionalType.getActualTypeArguments()[0], errorMessage);
        }
    }
